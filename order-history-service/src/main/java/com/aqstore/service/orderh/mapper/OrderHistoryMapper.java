...
public interface OrderHistoryMapper {
	...
	@Mapping(target = "lastUpdate", source = "lastModifiedDate", qualifiedByName = { "mapperConverter",
			"ldtToInstant" })
	...
	@Mapping(target = "available", source = "confirmed")
	@Mapping(target = "priceToUnit", source = "price")
	@Mapping(target = "weightToUnit", source = "weight")
	...
	default ApiOrdersDto toDto(Page<OrderHistory> ordersPage, Integer limit) {
		ApiOrdersDto dto = new ApiOrdersDto().totalItems(0L).itemsInCurrentPage(0).itemsLimitPerPage(limit).totalPage(0)
				.currentPage(0).hasNextPage(false).nextPage(null);
	...
	default OrderHistory toNewEntity(OrderHistoryEvent event) {
		return OrderHistory.builder()
				.creationDate(
						LocalDateTime.ofInstant(Instant.ofEpochMilli(event.getCreationDate()), ZoneId.systemDefault()))
				.idUser(event.getIdUser()).idOrder(event.getOrderId()).status(event.getStatus())
	...
	default void updateEntity(OrderHistory orderToUpdate, OrderHistoryEvent event) {
		...
		orderToUpdate.setLastModifiedDate(convertLongToLocalDateTime(event.getEventStepDate()));
		if (!event.getEventType().equals(EventType.CQRS_DELETED)) {
			...
			case STEP4_CONFIRM_PAYMENT:
				orderToUpdate.setPaymentDate(convertLongToLocalDateTime(event.getEventStepDate()));
			case STEP5_ORDER_SHIPPED:
				orderToUpdate.setShippingDate(convertLongToLocalDateTime(event.getEventStepDate()));
				break;
			case STEP6_ORDER_DELIVERED:
				orderToUpdate.setDeliveryDate(convertLongToLocalDateTime(event.getEventStepDate()));
				break;
			default:
				break;
			}
